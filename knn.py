...
Original file is located at
    https://colab.research.google.com/drive/1kdkQo3XnpfOBziAR2IS-tMSAC-wkqSQd
...
##**Estimando a dissipação de potência de FPGA através de dados de utilização de recursos**
...
#data = pd.read_table("https://drive.google.com/uc?export=view&id=1KnEHfBs488iEu6bXkU93YVvs60FLS3k5") # original
data = pd.read_table("https://drive.google.com/uc?export=view&id=12jKRSRz1dycrVxfnFWln9wA1KtjFZ5BF") # pre-processado
data = data.iloc[: , 1:5]
...
plt.hist(data['Power'])
plt.title("Distribuição do atributo alvo - Power")
plt.show()
...
data.drop(['Power'],axis=1).describe()

data.drop(['Power'],axis=1).hist(bins=15, figsize=(20,18))
plt.show()
...
> ***Sua resposta aqui:*** existem grandes discrepâncias entre os intervalos dos atributos, o que afeta negativamente os modelos criando viéses em relação a atributos de valores muito grandes.

É interessante também observarmos a correlação entre os atributos. Em alguns casos, pode ser vantajoso remover atributos de pares com alta correlação entre si (mas não iremos aprofundar no pré-processamento agora).
"""
...
input_atr1 = "FF" #@param {type:"string"}
input_atr2 = "LUT" #@param {type:"string"}
